...
class Pay extends Controller {
    ...
    /* Function to log payment errors for debugging */
    private function log_payment_error($processor, $error_details) {
        $log_entry = '[' . date('Y-m-d H:i:s') . '] - ' . $processor . ' - ' . $error_details . PHP_EOL;
        ...
        switch ($_POST['payment_type']) {
            ...
            case 'one_time':
                ...
                try {
                    ...
                    $product_name = settings()->business->brand_name . ' - ' . $this->plan->name . ' - ' . l('plan.custom_plan.' . $_POST['payment_frequency']);
                    ...
                    $price_id = $this->get_or_create_paddle_price($product_id, $formatted_price, currency(), $_POST['payment_frequency']);
                    ...
                    if ($transaction_response->code >= 400 || !isset($transaction_response->body->data)) {
                        ...
                        $this->log_payment_error('PADDLE', 'Transaction failed: ' . $error_message);
                    ...
                    $this->log_payment_error('PADDLE_EXCEPTION', $exception->getMessage());
    ...
    private function get_or_create_paddle_product($product_name) {
        ...
        if ($response->code >= 400 || !isset($response->body->data)) {
            $this->log_payment_error('PADDLE', 'Product creation failed: ' . json_encode($response->body));
            return null;
        }
    ...
    private function get_or_create_paddle_price($product_id, $amount, $currency, $billing_cycle) {
        ...
        if ($response->code >= 400 || !isset($response->body->data)) {
            $this->log_payment_error('PADDLE', 'Price creation failed: ' . json_encode($response->body));
            return null;
        }
